...
training_set = train_datagen.flow_from_directory(
    os.path.join(deepeye_path, "Images/cnnImages/BinaryClassifier/Training/eyetype"),
    target_size=(256, 256),
    batch_size=32,
)
...
with open(os.path.join(deepeye_path, "Logs/cnnLogs/binary_training_image_filenames.txt"), "w") as file:
    file.writelines(filename + "\n" for filename in training_image_filenames)

# Preprocessing the Test set
test_datagen = ImageDataGenerator(rescale=1.0 / 255)
test_set = test_datagen.flow_from_directory(
    os.path.join(deepeye_path, "Images/cnnImages/BinaryClassifier/Testing"), target_size=(256, 256), batch_size=32
)
...
filepath = os.path.join(deepeye_path, "Models/cnnModels/best_tl_binary_model.h5")
...
plt.savefig(os.path.join(deepeye_path, "Figures/cnnFigures/bi_training_accuracy_plot.png"))
plt.show()
...
plt.savefig(os.path.join(deepeye_path, "Figures/cnnFigures/bi_validation_accuracy_plot.png"))
plt.show()

# Save the output logs
with open(os.path.join(deepeye_path, "Logs/cnnLogs/binary_output_logs.txt"), "w") as log_file:
