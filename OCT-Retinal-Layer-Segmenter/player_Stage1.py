...
classNames = ['AGE_RMD','CSR','DIABETR','MACHOLE','NORMAL']
classDirs = ['AGE_RMD_55','CSR_102','DIABETR_107','MACHOLE_102','NORMAL_206']
cAbbr = ['AR', 'CR', 'DR', 'MH', 'NR']

imagesPathRead = '../Images/CT_RETINA/' + classDirs[testClass] + '/' + cAbbr[testClass] + 'Test'
imagesPathWrite ='../Images/CT_RETINA/' + classDirs[testClass] + '/' + cAbbr[testClass] + 'TempRes/'
...
nOrder =  [0, 5, 3, 6, 1, 7, 4, 2, 8] #Sequence of layers per model:
...
#for i in range(0,len(imageIds)):
    ...
    for j in range(0,9):        
        ...
        cv2.imwrite(imagesPathWrite + classNames[testClass] + str (i+1) + '_' + str(j) + '_' + layersNames[j] + '.jpg', layerTest)
